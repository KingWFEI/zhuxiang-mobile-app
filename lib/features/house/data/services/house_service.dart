...
class HouseService {
  ...
  Future<List<HouseTag>> fetchTags() async {
    ...
    if (result is ApiFailure<Response<dynamic>>) {
      throw result.error ?? ApiException(type: ApiExceptionType.unknown, message: result.message);
    }
    return const [];
  }
  ...
  void _checkResult(ApiResult<Response<dynamic>> result, String fallbackMsg) {
    if (result is ApiSuccess<Response<dynamic>>) {
      ...
      if (body is Map<String, dynamic>) {
        ...
        throw ApiException(type: ApiExceptionType.server, message: body['message']?.toString() ?? fallbackMsg);
      }
    }
    if (result is ApiFailure<Response<dynamic>>) {
      throw result.error ?? ApiException(type: ApiExceptionType.unknown, message: result.message);
  ...
  Future<PageResult<House>> getFavoriteHouses({int page = 1, int pageSize = 20}) async {
    ...
    if (result is ApiSuccess<Response<dynamic>>) {
      ...
      if (body is Map<String, dynamic>) {
        final code = body['code'] as int? ?? -1;
        if (code != 200) throw ApiException(type: ApiExceptionType.server, message: body['message']?.toString() ?? '获取收藏列表失败');
        ...
        if (data != null) {
          ...
          return PageResult(
            ...
            hasMore: ((data['page'] as int?) ?? 1) < ((data['totalPages'] as int?) ?? 1),
          );
        }
      }
    }
    throw const ApiException(type: ApiExceptionType.unknown, message: '获取收藏列表失败');
  }
}
