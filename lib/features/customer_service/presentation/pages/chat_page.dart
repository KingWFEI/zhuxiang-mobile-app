...
final _messagesProvider = FutureProvider.autoDispose
    .family<List<CsMessage>, String>((ref, sessionId) async {
      final api = ref.read(csApiProvider);
      final result = await api.getMessages(sessionId);
      if (result is ApiSuccess<List<CsMessage>>) {
        return result.data;
      }
      if (result is ApiFailure<List<CsMessage>>) {
...
class _ChatPageState extends ConsumerState<ChatPage> {
  ...
  Future<void> _enter() async {
    ...
    AppToast.show(
      context,
      errMsg.isNotEmpty ? errMsg : '客服服务暂不可用',
  ...
  void _sendMessage([String? text]) {
    ...
    _sseSub?.cancel();
    _sseSub = api
        .streamMessage(sessionId: _effectiveSessionId, message: t)
        .listen(
          (e) {
            if (!mounted) return;
            final j = e.jsonData;
  ...
  bool get _ready =>
      !_isEntering &&
      _effectiveSessionId.isNotEmpty &&
      _historyLoaded &&
      !_isSending;
  ...
  Widget _body(AsyncValue<List<CsMessage>> async) {
    ...
    final msgs = _historyLoaded
        ? _messages
        : (async is AsyncData<List<CsMessage>> ? async.value : <CsMessage>[]);
    final showLoading =
        !_historyLoaded && _messages.isEmpty && async is AsyncLoading;
  ...
  Widget _welcome() {
    return ListView(
      ...
      children: [
        ...
        const Center(
          child: Text('你好，我是勿忧管家智能客服', style: AppTextStyles.titleMedium),
        ),
        const SizedBox(height: AppSpacing.sm),
        const Center(
          child: Text(
            '可以帮你查询租约、账单、门锁、预约和报修问题',
